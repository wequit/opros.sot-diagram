...
const LoginForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Фокус на поле ввода имени пользователя с небольшой задержкой
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!credentials.username || !credentials.password) {
      setError(getTranslation("LoginForm_EmptyFields", language));
      return;
    }
    ...
    try {
      ...
      if (result && result.access) {
        ...
        setError(getTranslation("LoginForm_NoToken", language) || "Access токен отсутствует в ответе");
      }
    } catch (err) {
      console.error("Ошибка:", err);
      setError(err instanceof Error 
        ? err.message 
        : getTranslation("LoginForm_Error", language) || "Ошибка при входе");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Шапка */}
      <header className="bg-gradient-to-r from-blue-700 to-blue-900 text-white shadow-xl relative overflow-hidden z-10 
      ...
      <main className="flex-grow flex items-center justify-center py-8 px-4">
        <div className="w-full max-w-md transform transition-all duration-700 ease-out animate-float">
          <div className="relative bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl p-7 sm:p-8 border border-gray-200/50
                         hover:shadow-blue-500/10 transition-all duration-500 ease-in-out group">
            {/* Эффект свечения */}
            <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-400 to-blue-600 rounded-2xl blur-xl opacity-5 group-hover:opacity-15 transition duration-1000 
                          animate-glow"></div>
            ...
            <div className="text-center mb-6 mt-2 animate-slideUp animation-delay-300">
              ...
              <h2 className="text-2xl font-bold text-gray-800 mb-2">
                {getTranslation("LoginForm_LoginTitle", language)}
              </h2>
              <p className="text-gray-500 text-sm">
                {getTranslation("LoginForm_LoginSubtitle", language)}
              </p>
            </div>
            ...
            <form 
              ...
              className="space-y-5 animate-slideUp animation-delay-500"
            ...
            >
              ...
              {error && (
                <div role="alert" aria-live="assertive" className="p-4 rounded-lg text-sm bg-red-50 border-l-4 border-red-500 text-red-600 
              ...
              <button
                type="submit"
                disabled={isLoading}
                aria-label={isLoading ? getTranslation("LoginForm_Loading", language) : getTranslation("LoginForm_LoginButton", language)}
